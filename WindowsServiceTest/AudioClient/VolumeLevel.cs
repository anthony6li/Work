...
using NAudio.Lame;
using NAudio.Wave;
using AudioClient.Sources;
using AudioClient.Sources.Audio;
using AudioClient.Sources.Audio.streams;
...
namespace AudioClient
{
    public sealed partial class VolumeLevel : PictureBox, ISpyControl
    {
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            ...
            //                    case 0:
            //                        _toolTipMic.Show(
            //                            IsEnabled ? "switchOff" : "Switchon", this,
            ...
            //                        {
            //                            _toolTipMic.Show("RecordNow", this, toolTipLocation, 1000);
            //                            _ttind = 1;
            //                        }
            //                        break;
            //                    case 2:
            //                        _toolTipMic.Show("Edit", this, toolTipLocation, 1000);
            //                        _ttind = 2;
            //                        break;
            //                    case 3:
            //                        if (Helper.HasFeature(Enums.Features.Access_Media))
            //                        {
            //                            _toolTipMic.Show("MediaoverTheWeb", this, toolTipLocation, 1000);
            //                            _ttind = 3;
            //                        }
            //                        break;
            //                    case 4:
            ...
            //                            ? "StopListening"
            //                            : "Listen", this, toolTipLocation, 1000);
            //                        _ttind = 4;
        ...
        public string[] ScheduleDetails
        {
            get
            {
                ...
                foreach (var sched in Micobject.schedule.entries)
                {
                    ...
                    daysofweek = daysofweek.Replace("0", "Sun");
                    daysofweek = daysofweek.Replace("1", "Mon");
                    daysofweek = daysofweek.Replace("2", "Tue");
                    daysofweek = daysofweek.Replace("3", "Wed");
                    daysofweek = daysofweek.Replace("4", "Thu");
                    daysofweek = daysofweek.Replace("5", "Fri");
                    daysofweek = daysofweek.Replace("6", "Sat");
        ...
        {
            if (IsEnabled && AudioSource != null && !IsClone && !IsReconnect)
            {
        ...
        private void DrawButton(Graphics gCam, int buttonIndex)
        {
            ...
            gCam.DrawImage(Properties.Resources.icons, rDest, rSrc, GraphicsUnit.Pixel);
        }
        ...
        protected override void OnPaint(PaintEventArgs pe)
        {
            ...
            if (IsEnabled)
            {
                ...
                if (l != null && !AudioSourceErrorState)
                {
                    ...
                    if (bh <= 2)
                        bh = 2;
                    using (var lgb = new SolidBrush(Color.Yellow))
                    {
                        ...
                        if (Listening)
                        {
                            gMic.DrawString("LIVE", new Font("Î¢ÈíÑÅºÚ",12), new SolidBrush(Color.Gray), new PointF(5, 4));
                        }


                        if (Recording)
                        {
                            gMic.FillEllipse(new SolidBrush(Color.Red), new Rectangle(rc.Width - 14, 2, 8, 8));
                        }
                    }
                }
                else
            ...
            {
                txt += ": " + "Offline";
                gMic.DrawString(SourceType + ": " + Micobject.name, new Font("Î¢ÈíÑÅºÚ",12), new SolidBrush(Color.Black), new PointF(5, 5));
            }
            ...
            gMic.DrawString(m + txt, new Font("Î¢ÈíÑÅºÚ", 12), new SolidBrush(Color.Black), new PointF(5, rc.Height - 18));
            


            if (_mouseMove > Helper.Now.AddSeconds(-3))
            {
                DrawOverlay(gMic);
            }
        ...
        private void DrawOverlay(Graphics gMic)
        {
            ...
            gMic.FillRectangle(new SolidBrush(Color.Gray), rPanel);
            ...
            try
            {
                ...
                //{
                    ...
                    //case 1: //ispy server (fixed waveformat at the moment...)
                        AudioSource = new iSpyServerStream(Micobject.settings.sourcename,ParentForm)
                ...
                if (AudioSource != null)
                {
                    ...
                    if (!AudioSource.IsRunning && !IsClone)
                    {
                        lock (_lockobject)
                        {
                            AudioSource.Start();
        ...
        {
            if (!Helper.HasFeature(Enums.Features.Recording))
                return "notrecording," + "RecordingStopped";
            if (record)
            {
                if (!IsEnabled)
                {
                ...
                ForcedRecording = true;
                _requestRefresh = true;
                return "recording," + "RecordingStarted";
            }
