...
using AudioServer1.Sources;
using AudioServer1.Sources.Audio;
using AudioServer1.Sources.Audio.streams;

namespace AudioServer1
{
    public sealed partial class SendVolumeLevel : PictureBox
    {
        #region Private
        ...
        public SendVolumeLevel(objectsMicrophone om, int mode, MainForm form)
        {
            InitializeComponent();
        ...
        protected override void OnPaint(PaintEventArgs pe)
        {
            ...
            if (Micobject.settings.active)
            {
                ...
                if (audioMode == 0)
                {
                    gMic.DrawString("http://" + ParentForm.AddressIPv4 + ":" + ParentForm.nPort + "/?micid=" + Micobject.id, drawFont, drawBrush,
            ...
            else
            {
                if (NoSource)
                {
                    gMic.DrawString("NoSource" + ": " + Micobject.name,
                ...
                else
                {
                    ...
                    {
                        gMic.DrawString("Scheduled" + ": " + Micobject.name,
                                         drawFont, drawBrush, new PointF(5, 5));
                    }
                    else
                    {
                        gMic.DrawString("Inactive" + ": " + Micobject.name,
                                         drawFont, drawBrush, new PointF(5, 5));
                    }
                }
            }
        ...
        internal void SendVolumn(string str)
        {
            lock (thisLock)
            {
                ...
                if (OutSockets.Count > 0)
                {
                    //var enc = new byte[byteArray.Length/2];
                    //ALawEncoder.ALawEncode(byteArray, enc);
                    ...
                    for (int i = 0; i < OutSockets.Count; i++)
                    {
                        ...
                        if (s.Connected)
                        {
                            sendTextToForm(string.Format("[{0}] from [{1}]to [{2}]\r\n", str, s.LocalEndPoint, s.RemoteEndPoint));
                            if (!SendToBrowser(byteArray, s))
                            {
                                OutSockets.Remove(s);
                                i--;
                            }
                        ...
                        {
                            sendTextToForm(string.Format("Remote client [{0}] is Disconnected.\r\n", s.RemoteEndPoint));
                            OutSockets.Remove(s);
                            i--;
                        }
                    }
                ...
                {
                    sendTextToForm(string.Format("No socket connection.\r\n"));
                }
        ...
        private delegate void sendTextCallBack(string mes);
        private void sendTextToForm(string mes)
        {
            if (this.InvokeRequired)
            {
                sendTextCallBack st = new sendTextCallBack(sendTextToForm);
                ParentForm.Invoke(st, new object[] { mes });
            }
            else
            {
                ParentForm.richTextBox1.Text += string.Format(mes);
            }
        }
