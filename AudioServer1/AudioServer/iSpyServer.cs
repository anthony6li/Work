...
namespace AudioServer1
{
    ...
    public class iSpyLANServer
    {
        ...
        {       
            ...
            {
                List<int> tempClientIds = MainForm.clientId;
                ...
                try
                {
                    ...
                    if (mySocket.Connected)
                    {
                        ...
                        try
                        {
                            ...
                            //判断客端传来的ID在Server的集合中，不存在就不发送数据
                            if ((vid != -1)&&(tempClientIds.Contains(vid)))
                            {
        ...
        public void SendToBrowser(Byte[] bSendData, Socket socket)
        {
            try
            {
                if (socket.Connected)
                {
                    ...
                    {
                        Console.WriteLine("Only sent " + sent + " of " + bSendData.Length);
                    }
                    //if (sent == -1)
                        //MainForm.LogExceptionToFile(new Exception("Socket Error cannot Send Packet"));
                }
